import React from "react";
import BackendSpecialistImg from "../../assets/images/backendSpecialist.png";

const skills = [
  "API Deployment",
  "API Security",
  "API Testing",
  "Advanced Data Structures",
  "Advanced Flask",
  "CI/CD with GitHub Actions",
  "GraphQL",
  "Python Data Structures",
  "REST API Design",
  "SQL & ORM",
  "WebSocket",
];

export default function BackendSpecialization() {
  return (
    <main className="min-h-screen w-full bg-gradient-to-br from-white via-purple-100 to-purple-300 flex items-center justify-center px-6 py-6">
      <div className="w-full flex flex-col md:flex-row-reverse items-stretch">
        {/* Right side: Image */}
        <div className="flex-1 flex items-center justify-center bg-purple-100 min-h-[350px] md:min-h-[500px]">
          <img
            src={BackendSpecialistImg}
            alt="Back-End Specialization Certificate Badge"
            className="w-full h-[350px] md:h-[500px] object-contain rounded-xl shadow-xl"
          />
        </div>

        {/* Left: Certification Info */}
        <div className="flex-1 flex flex-col justify-center p-8 md:p-16 bg-white/90">
          <h1 className="text-4xl font-extrabold text-purple-800 mb-4">
            Back-End Specialization
          </h1>

          <p className="text-gray-700 text-lg mb-8">
            Earners of this badge have successfully completed the Back-End
            Specialization at Coding Temple, mastering skills in Python, database
            management, API development, and CI/CD practices. This certification
            demonstrates deep understanding of advanced RESTful API design,
            WebSocket implementation, GraphQL API development, and backend
            architecture through hands-on mini-projects culminating in an
            e-commerce API project.
          </p>

          <div className="flex flex-wrap gap-3 mb-8">
            <p className="text-lg text-purple-700 font-semibold">
              Skills / Knowledge:
            </p>

            {skills.map((skill) => (
              <span
                key={skill}
                className="inline-block px-4 py-2 rounded-full bg-gradient-to-tr from-purple-200 via-purple-100 to-purple-300 text-purple-900 font-semibold shadow-md border border-purple-300 hover:scale-105 transition"
              >
                {skill}
              </span>
            ))}
          </div>

          <div className="flex justify-center">
            <a
              href="https://www.credential.net/c6f62306-bf5f-4883-b915-ce53574b492b#acc.zdOeiqjR"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-block px-4 py-2 bg-purple-700 text-white font-semibold rounded-md shadow hover:bg-purple-600 transition text-sm"
            >
              View Certificate
            </a>
          </div>
        </div>
      </div>
    </main>
  );
}
